...
import itu.p16.gestion.finance.service.AnalyseService;
import itu.p16.gestion.finance.service.EtatFinancierService;
import itu.p16.gestion.finance.service.SolutionService;
...
public class AnalyseController {
    ...
    @GetMapping("/analyse")
    public String showAnalyseForm() { 
        return "analyse";  
    }

    @PostMapping("/analyse")
    public String analyseFinancialData(@RequestParam("date") String date, Model model)  {
        ...
        try {
           ...
           for (Categorie categorie : categories) {
            ...
            if (avertissements.size() > 0) {
                String error = "Les conditions ne sont pas respectees pour effectuer l'analyse : ";
                for (String string : avertissements) {
                    error = error + string + "; ";
                }
                throw new Exception(error);
            }
            Map<String, Double> valeurs = analyseService.calculerAnalyses(categories);
            System.out.println(valeurs);
            List<Analyse> analyses = analyseService.getAnalyses(valeurs);

            // String conclusion = solutionService.generateGlobalConclusion(analyses);
